...
/**
 * Builds a test-only Express app that mounts the REAL video-upload router,
 * controller and services, but injects the `req.context` the controller reads.
 *
...
export function buildUploadTestApp(): Express {
  ...
  const videoUploadService = new VideoUploadsService(
    { presignedUrlTtlSeconds: 900, uploadPartSizeBytes: 5 * 1024 * 1024 },
    fileUpload,
  )
  ...
  app.use((req: Request, _res: Response, next: NextFunction) => {
    ...
    // what the controller actually reads at runtime.
    ;(req as any).context = { services: { videoUploadService } }
    next()
  })
