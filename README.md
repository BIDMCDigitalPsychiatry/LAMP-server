...
2. **Participant Sessions Do Not Expire** -- Participant sessions should never expire, while staff sessions should expire after a sensible ammount of time. In order to ensure this is the case we manually adjust the expiry date of session cookies when they are created. We also rotate the participant session when a participant requests a new session cookie using the mobile session refresh endpoint.
...
4. **Three types of login keys** -- In order for the server to support web use, data analysis using python, and the mobile app there are three types of authentication keys:
