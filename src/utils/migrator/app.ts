...
import { Setup, Changes, Researcher, Study, Participant, Activity, ActivityEvent, SensorEvent, Credential } from './migrators'
...
export default async function migrator() {
	...
	async function createDB(name: string, documents?: (() => Promise<any[]>)) {
		...
		if (!!documents) {
			console.dir(await nano.use(name).bulk({ 
	...
	async function biDiSyncUnit(name: string, backward: (change_version: string) => Promise<string>, forward: (change_version: string) => Promise<string>) {
		console.group()
		console.time()
		let token = (await nano.use('root').get('sync_token')) as any
		...
		if ((token.cdb[name] !== token_bak.cdb[name]) || (token.sql[name] !== token_bak.sql[name]))
			await updateToken(token)
	...
	async function oneWaySync() {
		...
		if (!existsDB('researcher')) 
			await createDB('researcher', async () => await Researcher._select())
		if (!existsDB('participant'))
			await createDB('participant', async () => await Participant._select())
		if (!existsDB('activity'))
			await createDB('activity', async () => await Activity._select())
		if (!existsDB('activity_event'))
			await createDB('activity_event', async () => await ActivityEvent._select())
		if (!existsDB('sensor_event'))
			await createDB('sensor_event', async () => await SensorEvent._select())
		console.log('-- FINISHED ONEWAY SYNC --')
	}
	async function biDiSync() {
		console.log('-- STARTING BIDI SYNC --')
		if (existsDB('researcher')) {
			await biDiSyncUnit('researcher', async (change_version) => {
				...
				console.dir(await nano.use('researcher').bulk({ 
					docs: results
							.map(x => ({ ...x, 
								$_rec: reconcilations.find(y => y.SourceID === x._id && y.Action === 'INSERT')
							}))
							.filter(x => !!x.$_rec)
							.map(x => ({ ...x, $_rec: undefined,
								$_sync_id: x.$_rec.TargetID
							}))
				}))
		...
		if (existsDB('participant')) {
			await biDiSyncUnit('participant', async (change_version) => {
				...
				console.dir(await nano.use('participant').bulk({ 
		...
		if (existsDB('activity')) {
			await biDiSyncUnit('activity', async (change_version) => {
				...
				console.dir(await nano.use('activity').bulk({ 
		...
		if (existsDB('activity_event')) {
			await biDiSyncUnit('activity_event', async (change_version) => {
				...
				console.dir(await nano.use('activity_event').bulk({ 
		...
		if (existsDB('sensor_event')) {
			await biDiSyncUnit('sensor_event', async (change_version) => {
				// NOTE: Does not handle INSERT, UPDATE or DELETE operations.
				return change_version
			}, async (change_version) => {
				// NOTE: Does not handle UPDATE or DELETE operations.
				console.dir(await nano.use('sensor_event').bulk({ 
