...
export default abstract class ScriptRunner {
	...
	public static Bash = class extends ScriptRunner {
		async execute(script: string, requirements: string, input: any): Promise<any> {
			return new Promise((resolve, reject) => ScriptRunner.queue.enqueue(async () => {
				...
				try {
					...
					await container.putArchive(makeTar({ 
						'/src/script': script,
					}), { path: '/' })
			...
			return new Promise((resolve, reject) => ScriptRunner.queue.enqueue(async () => {
				...
				try {
					...
					if (requirements.split(',').length > 0) {
						...
						logs.push(await containerExec(container, `apt-get update -qq && apt-get install -y ${requirements.split(',').map(x => 'r-cran-' + x.trim()).join(' ')}; ls /usr/lib/R/site-library/; /usr/lib/R/site-library/littler/examples/install2.r -s ${requirements.split(',').join(' ')}`))
