  ...
  if (undefined !== req.query.researcher_id) {
    ...
    await nc.subscribe(`researcher.*`, async (err, msg) => {
      if (err) {
        ...
        if (!!JSON.parse(JSON.parse(msg.data).data).subject_id) {
          if (researcher_id === JSON.parse(JSON.parse(msg.data).data).subject_id) {
            try {
              if (JSON.parse(JSON.parse(msg.data).data).action !== "delete") {
                const ID: any = JSON.parse(JSON.parse(msg.data).data).subject_id ?? ""
                if (!!ID) {
                  ...
                  if (!!data[0]) {
                    ...
                    let data_: any = {}
                    data_ = JSON.parse(msg.data)
                    data_ = { ...JSON.parse(data_.data), name: data[0].name, researcher_id: ID, topic: "researcher.*" }
                    let Data: any = {}
                    //APPENDING DATA
...
ListenerAPI.get("/researcher/study", async (req: Request, res: Response) => {
  ...
  if (undefined !== req.query.researcher_id) {
    let researcher_id = req.query.researcher_id
    let sub = await nc.subscribe(`researcher.*.study`, async (err, msg) => {
      if (err) {
        console.log("error while subscribing", msg)
      } else {
        //IF SUBJECT ID IS GIVEN, TAKE DATA FROM DB USING THE PUBLISHED ID AND SEND (IF SIZE GREATER THAN NATS PAYLOAD MAX SIZE)
...
ListenerAPI.get("/study/activity", async (req: Request, res: Response) => {
  ...
  if (undefined !== req.query.activity_id) {
    ...
    let sub = await nc.subscribe(`activity.*`, async (err, msg) => {
      if (err) {
        console.log("error while subscribing", msg)
      } else {
        //IF SUBJECT ID IS GIVEN, TAKE DATA FROM DB USING THE PUBLISHED ID AND SEND (IF SIZE GREATER THAN NATS PAYLOAD MAX SIZE)
      ...
      if (err) {
        ...
        if (!!JSON.parse(JSON.parse(msg.data).data).subject_id) {
          try {
            if (JSON.parse(JSON.parse(msg.data).data).action !== "delete") {
              const ID: any = JSON.parse(JSON.parse(msg.data).data).subject_id ?? ""
              const timestamp: any = JSON.parse(JSON.parse(msg.data).data).timestamp ?? ""
              const activityID: any = JSON.parse(JSON.parse(msg.data).data).activity ?? ""
              if (!!ID) {
                let data: any[] = []
                try {
                  data = await ActivityEventRepository._select(ID, activityID, timestamp)
                  if (participant_id === JSON.parse(JSON.parse(msg.data).data).subject_id) {
        ...
        if (!!JSON.parse(JSON.parse(msg.data).data).subject_id) {
          try {
            if (JSON.parse(JSON.parse(msg.data).data).action !== "delete") {
              const ID: any = JSON.parse(JSON.parse(msg.data).data).subject_id ?? ""
              const timestamp: any = JSON.parse(JSON.parse(msg.data).data).timestamp ?? ""
              const sensorID: any = JSON.parse(JSON.parse(msg.data).data).sensor ?? "" //LAMP.acc...
              if (!!ID) {
                let data: any[] = []
                try {
      ...
      if (err) {
       ...
       //IF SUBJECT ID IS GIVEN, TAKE DATA FROM DB USING THE PUBLISHED ID AND SEND (IF SIZE GREATER THAN NATS PAYLOAD MAX SIZE)
        if (!!JSON.parse(JSON.parse(msg.data).data).subject_id) {
          ...
          if (sensor.toLowerCase() === JSON.parse(JSON.parse(msg.data).data).sensor.toLowerCase()) {
            //generate event id
            const _id = new Date().toLocaleTimeString()
            const data = msg.data
            //Send SSE
    ...
    await nc.subscribe(`participant.*.sensor.*.sensor_event`, async (err, msg) => {
      if (err) {
        ...
        if (!!JSON.parse(JSON.parse(msg.data).data).subject_id) {
          ...
          if (
            participant_id === JSON.parse(JSON.parse(msg.data).data).participant_id &&
            sensor.toLowerCase() === JSON.parse(JSON.parse(msg.data).data).sensor.toLowerCase()
          ) {
            //generate event id
            const _id = new Date().toLocaleTimeString()
            const data = msg.data
        ...
        if (!!JSON.parse(JSON.parse(msg.data).data).subject_id) {
          try {
            if (JSON.parse(JSON.parse(msg.data).data).action !== "delete") {
              const ID: any = JSON.parse(JSON.parse(msg.data).data).subject_id ?? ""
              const timestamp: any = JSON.parse(JSON.parse(msg.data).data).timestamp ?? ""
              const sensorID: any = JSON.parse(JSON.parse(msg.data).data).sensor ?? ""
              if (!!ID) {
                let data: any[] = []
                try {
                  data = await SensorEventRepository._select(ID, sensorID, timestamp)
