...
import { Part, PresignedUrlInfoWithExpiration } from "../service/AudioUploadService";
// NOTE: schemas/fileUpload holds the generic (media-agnostic) upload request/
// response shapes shared by both the audio and video upload flows.
import {
  AbortBody,
  CompleteBody,
  CompleteResponse,
...
export interface AudioUploadController {
...
// middleware (see AudioUploadRouter). Handlers read the already-parsed,
// typed data from `req.validated.*`.

export class AudioUploadControllerImpl implements AudioUploadController {

  async abort(req: Request, resp: Response): Promise<void> {
    const { id } = req.validated.body as AbortBody

    await req.context.services.audioUploadService.abort(id)
  ...
  async complete(req: Request, resp: Response<CompleteResponse>): Promise<void> {
    ...
    await req.context.services.audioUploadService.complete(id, parts)
  ...
  async initiate(req: Request, resp: Response<InitiateResponse>): Promise<void> {
    ...
    const id = await req.context.services.audioUploadService.initiate({
    ...
    const partRanges : Part[] = req.context.services.audioUploadService.calculatePartRanges(fileSizeBytes)
    const partUrls : PresignedUrlInfoWithExpiration[] = await req.context.services.audioUploadService.getUploadUrlParts(id, map(partRanges, "partNumber"))
  ...
  async refreshUrls(req: Request, resp: Response<RefreshUrlsResponse>): Promise<void> {
    ...
    const partUrls : PresignedUrlInfoWithExpiration[] = await req.context.services.audioUploadService.getUploadUrlParts(id, partNumbers)
