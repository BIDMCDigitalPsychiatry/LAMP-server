...
export async function _migrate_sensor_event() {
  ...
  try {
    ...
    const next_version =
      Number.parse((await SQL!.query`SELECT CHANGE_TRACKING_CURRENT_VERSION()`).recordset[0][""]) ?? 0
	...
	;`)
      ...
      obj.sensor = Object.entries(HK_LAMP_map).filter((x) => x[1] === (raw.type as string))[0][0] as SensorName
      obj.data = ((HK_to_LAMP as any)[obj.sensor!] || ((x: any) => x))(raw.data)
      return obj
    })

    console.dir("[HealthKit_Values] migrating " + result1.length + " events")
    for (let i = 0; i < Math.ceil(result1.length / 30_000); i++) {
      const out = await Database.use("sensor_event").bulk({ docs: result1.slice(i * 30_000, (i + 1) * 30_000) })
    ...
    for (let i = 0; i < Math.ceil(result2.length / 30_000); i++) {
      const out = await Database.use("sensor_event").bulk({ docs: result2.slice(i * 30_000, (i + 1) * 30_000) })
      console.dir(out.filter((x) => !!x.error))
    }
...
export async function _migrate_activity_event(): Promise<void> {
  if (_migrator_active) {
    return console.info("activity_event migration aborted due to existing migrator activity")
  }
  _migrator_active = true
  try {
    const MigratorLink = Database.use("root")
    ...
    if (change_version === undefined) {
      return console.info("activity_event migration aborted due to missing change_version token")
    }
    const next_version =
      Number.parse((await SQL!.query`SELECT CHANGE_TRACKING_CURRENT_VERSION()`).recordset[0][""]) ?? 0
    change_tracking.db.activity_event = next_version
    if (next_version - change_version === 0) {
      return console.info("activity_event migration aborted due to no tracked changes")
    }
    console.dir(`activity_event migration from ${change_version ?? 0} to ${next_version ?? 0}`)
      ...
      const res = events.map((row: any) => {
        ...
        activity_event.temporal_slices = JSON.parse(row.slices ?? "[]").map((slice_row: any) => {
          ...
          if (entry.LegacyCTestID === null) {
            ...
            temporal_slice.type = !temporal_slice.type ? undefined : (temporal_slice.type as string).toLowerCase()
    ...
    const all_res = ([] as ActivityEvent[]).concat(...(await Promise.all(result)))
    for (let i = 0; i < Math.ceil(all_res.length / 30_000); i++) {
      const out = await Database.use("activity_event").bulk({ docs: all_res.slice(i * 30_000, (i + 1) * 30_000) })
      console.dir(out.filter((x) => !!x.error))
    }
