...
export class TypeRepository {
  public static async _parent(type_id: string): Promise<{}> {
    ...
    for (const parent_type of TypeRepository._parent_type(type_id))
      result[parent_type] = await TypeRepository._parent_id(
        type_id,
        parent_type,
      );
    return result;
  ...
  public static _self_type(type_id: string): string {
    ...
    const from_type: string =
      components.length === 0 ? (<any>Participant).name : components[0];
    return from_type;
  }
  ...
  public static async _parent_id(
    ...
    return await (<any>(
      self_type[TypeRepository._self_type(type_id)]
    ))._parent_id(type_id, self_type[type]);
  }

  /**
   *
   */
  public static async _set(
    mode: 'a' | 'b',
    type: string,
    id: string,
    key: string,
    value?: DynamicAttachment | any,
  ) {
  ...
  public static async _get(
    ...
    id: string,
    key: string,
  ): Promise<DynamicAttachment[] | any | undefined> {
  ...
  public static async _process_triggers() {
    ...
    const accumulated_set = (
      ...
      _id:
        x.Type === 'Participant'
          ? ParticipantRepository._pack_id({
              study_id: Decrypt(<string>x._SID),
            })
    ...
    let working_set = registered_set.filter(
      (x) =>
        ...
        (x.ChildObjectType === 'Participant' &&
          ax_set2.indexOf(x.ObjectID) >= 0) ||
        /* Attachment from self -> specific child Participant matching an ID */
        accumulated_set.find(
          (y) => y._id === x.ChildObjectType && y._admin === x.ObjectID,
        ) !== undefined,
    );
