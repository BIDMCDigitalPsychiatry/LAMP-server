...
export class CredentialRepository {
  ...
  public static async _showAll(): Promise<any[]> {
    ...
    const out1 = result1.recordset.map((x) => ({
      origin: ResearcherRepository._pack_id({
        admin_id: Number.parse(x['AdminID']) ?? 0,
      }),
  ...
  public static async _find(
    ...
    if (result.rowsAffected[0] > 0) {
      if (
        ...
        secret_key !==
          Decrypt(
            JSON.parse(result.recordset[0]['Value'])['secret_key'],
            'AES256',
          )
      )
    ...
    if (result.rowsAffected[0] > 0) {
      if (
        !!secret_key &&
        secret_key !== Decrypt(result.recordset[0]['Password'], 'AES256')
      )
        throw new Error('403.no-such-credentials');
      return ResearcherRepository._pack_id({
        admin_id: Number.parse(result.recordset[0]['AdminID']) ?? 0,
      });
    }
  ...
  public static async _select(type_id: string): Promise<string[]> {
    ...
    if (!!type_id && Identifier_unpack(type_id)[0] === (<any>Researcher).name)
      ...
      !!type_id &&
      Identifier_unpack(type_id).length === 0 /* Participant */
    )
