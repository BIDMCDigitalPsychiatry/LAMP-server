...
export class ParticipantRepository {
  ...
  public static async _insert(
    ...
    const study_code = !!object.study_code
      ? `'${Encrypt(object.study_code)}'`
      : `'${Encrypt('001')}'`;
    const theme = !!object.theme
      ? `'${Encrypt(object.theme!)}'`
      : `'dJjw5FK/FXK6qU32frXHvg=='`;
    const language = !!object.language
      ? `'${Encrypt(object.language!)}'`
      : `'en'`;
    const emergency_contact = !!object.emergency_contact
      ? `'${Encrypt(object.emergency_contact!)}'`
      : `''`;
    const helpline = !!object.helpline
      ? `'${Encrypt(object.helpline!)}'`
      : `''`;
  ...
  public static async _delete(
    ...
    const user_id = Encrypt(
      ParticipantRepository._unpack_id(participant_id).study_id,
    );
