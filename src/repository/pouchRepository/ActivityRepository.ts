...
export class ActivityRepository {
   ...
   *
   */
  public static async _parent_id(
    id: string,
    type: Function,
  ...
  public static async _select(
    ...
    if (!!id && Identifier_unpack(id)[0] === (<any>Researcher).name)
      ...
      admin_id = ResearcherRepository._unpack_id(
        (<any>await TypeRepository._parent(<string>id))['Researcher'],
      ).admin_id;
    ...
    const resultSurveySchedule = (
      await SQL!.request().query(`
			...
			WHERE IsDeleted = 0 
				AND Admin_SurveySchedule.SurveyID IN (${
          resultSurvey.length === 0
            ? 'NULL'
            : resultSurvey.map((x) => x.id).join(',')
        })
    ...
    const resultTestSchedule = (
      ...
      JOIN Admin_CTestSettings 
        ...
        AND Admin_CTestSettings.CTestID = Admin_CTestSchedule.CTestID
			WHERE IsDeleted = 0 
				AND AdminCTestSettingID IN (${
          resultTest.length === 0 ? 'NULL' : resultTest.map((x) => x.id)
        })
		;`)
    ).recordset;
      ...
      if (raw.type === 'batch') {
        ...
        obj.schedule = resultBatchSchedule
          ...
          .map((x) => ({
            ...x,
            id: undefined,
            custom_time: !x.custom_time
              ? null
              : JSON.parse(x.custom_time).map((y: any) => y.t),
          })) as any;
        ...
        obj.settings = resultSurveyQuestions
          ...
          .map((x) => ({
            ...
            options: !x.options
              ? null
              : JSON.parse(x.options).map((y: any) => y.opt),
          })) as any;
        ...
        const specEntry = ActivityIndex.find(
          (x) => x.LegacyCTestID === Number.parse(raw.ctest_id),
        );
    ...
    try {
      ...
      if (object.spec === 'lamp.group' /* group */) {
        ...
        if (result1.rowsAffected[0] !== object.schedule.length)
          throw new Error(
            '400.create-failed-due-to-malformed-parameters-schedule',
          );
        const batch_id =
          Number.parse(result1.recordset[0]['AdminBatchSchId']) ?? -1;
        ...
        if (ctime.length > 0) {
          ...
          if (result2.rowsAffected[0] === 0)
            throw new Error(
              '400.create-failed-due-to-malformed-parameters-timing',
            );
        }

        // Get CTest spec list.
        const items = object.settings.map((x, idx) => ({
        ...
        if (items.filter((x) => x.ctest_id > 0).length > 0) {
          ...
          if (
            ...
            items.filter((x) => x.ctest_id > 0).length
          )
            throw new Error(
              '400.create-failed-due-to-malformed-parameters-settings',
            );
        }
        if (items.filter((x) => x.survey_id > 0).length > 0) {
          ...
          const result4 = await transaction.request().query(`
              ...
              .join(', ')}
					;`);
          if (
            result4.rowsAffected[0] !==
            items.filter((x) => x.survey_id > 0).length
          )
            throw new Error(
              '400.create-failed-due-to-malformed-parameters-settings',
            );
        ...
        const result1 = await transaction.request().query(`
					...
					VALUES (${admin_id}, '${
          object.name ? _escapeMSSQL(object.name) : 'new_survey'
        }')
				;`);
        ...
        const ctest_id =
          ActivityIndex.find((x) => x.Name === object.spec)?.[0]
            ?.LegacyCTestID ?? -1;
        ...
        const _actual_setting_id =
          Number.parse(result.recordset[0]['AdminCTestSettingID']) ?? 0;
  ...
  public static async _update(
    ...
    const { ctest_id, survey_id, group_id } = ActivityRepository._unpack_id(
      activity_id,
    );
      ...
      if (group_id > 0 /* group */) {
        ...
        if (Array.isArray(object.schedule) || typeof object.name === 'string') {
          ...
          const result1 = await transaction.request().query(`
                ...
                !!object.schedule[0].start_date
                  ? `ScheduleDate = '${object.schedule[0].start_date}'`
                  : null,
                !!object.schedule[0].time
                  ? `Time = '${object.schedule[0].time}'`
                  : null,
          ...
          if (Array.isArray(object.schedule[0].custom_time)) {
            ...
            const result2 = await transaction.request().query(`
                  ...
                  ctime.length === 0
                    ? '(NULL, NULL)'
                    : ctime
                        .map((x: any) => `(${group_id}, '${_escapeMSSQL(x)}')`)
                        .join(', ')
                }
        ...
        if (Array.isArray(object.settings)) {
          ...
          if (items.filter((x) => x.group_id === 0).length === 0)
            throw new Error(
              '400.empty-activity-group-settings-array-unsupported',
            );
          ...
          const result3 = await transaction.request().query(`
                ...
                ctest.length === 0
                  ...
                  : ctest
                      .map(
                        (x) =>
                          `(${group_id}, (${_ctest_select}), ${x.idx + 1})`,
                      )
                      .join(', ')
              }
