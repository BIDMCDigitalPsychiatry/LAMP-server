...
export class TypeRepository implements TypeInterface {
  ...
  public async _get(mode: any, type_id: string, attachment_key: string): Promise<any | undefined> {
    const repo = new Repository()
    const TypeRepository = repo.getTypeRepository()
    const self_type = (type_id ===null || type_id ==='*') ? undefined : await TypeRepository._self_type(type_id)
    const parents = (type_id ===null || type_id ==='*') ? new Array : Object.values(await TypeRepository._parent(type_id)).reverse() 
