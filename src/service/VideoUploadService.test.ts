...
import { VideoUploadsService } from "./VideoUploadService"
...
describe("VideoUploadsService constructor validation", () => {
  it("accepts the lower-bound part size (5 MiB)", () => {
    expect(
      () => new VideoUploadsService({ ...validConfig, uploadPartSizeBytes: 5_242_880 }, makeUploadServiceMock()),
    ).not.toThrow()
  })

  it("accepts the upper-bound part size (10 MiB)", () => {
    expect(
      () => new VideoUploadsService({ ...validConfig, uploadPartSizeBytes: 10_485_760 }, makeUploadServiceMock()),
    ).not.toThrow()
  })

  it("throws below the minimum part size", () => {
    expect(
      () => new VideoUploadsService({ ...validConfig, uploadPartSizeBytes: 5_242_879 }, makeUploadServiceMock()),
    ).toThrow()
  })

  it("throws above the maximum part size", () => {
    expect(
      () => new VideoUploadsService({ ...validConfig, uploadPartSizeBytes: 10_485_761 }, makeUploadServiceMock()),
    ).toThrow()
  })

  it("throws for a non-integer part size", () => {
    expect(
      () => new VideoUploadsService({ ...validConfig, uploadPartSizeBytes: 5_242_880.5 }, makeUploadServiceMock()),
    ).toThrow()
  })
})

describe("calculatePartRanges", () => {
  const svc = new VideoUploadsService(validConfig, makeUploadServiceMock())
...
describe("initiate", () => {
  it("mints a vid-upl- id and delegates to uploadService.initiate with the videos/<id> path", async () => {
    const upload = makeUploadServiceMock()
    const svc = new VideoUploadsService(validConfig, upload)
    const metadata = { LAMP_PARTICIPANT_ID: "part1", FILE_SIZE_BYTES: "1000" }

    const id = await svc.initiate(metadata)

    expect(id).toMatch(/^vid-upl-[0-9a-f-]{36}$/)
    expect(upload.initiate).toHaveBeenCalledTimes(1)
    expect(upload.initiate).toHaveBeenCalledWith(`videos/${id}`, "video/mp4", metadata)
  })
})
...
describe("getUploadUrlParts", () => {
  ...
  it("stamps presignedUrlExpiration = floor(now/1000) + ttl and forwards the ttl", async () => {
    ...
    const svc = new VideoUploadsService(validConfig, upload)
    const result = await svc.getUploadUrlParts("vid-upl-abc", [1, 2])

    expect(upload.getPresignedUrlInfoForParts).toHaveBeenCalledWith("videos/vid-upl-abc", [1, 2], 900)
