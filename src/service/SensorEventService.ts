...
import { SensorEventRepository as SensorPouchRepo } from '../repository/pouchRepository/SensorEventRepository';
import jsonata from 'jsonata';

export const SensorEventService = Router();
SensorEventService.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (process.env.LOCAL_DATA === 'true') {
        const outputLocal = {
          data: await SensorPouchRepo._insert(
            participant_id,
...
SensorEventService.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const output = {
        data: await SensorEventRepository._delete(
          participant_id,
          origin,
          from,
          to,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (process.env.LOCAL_DATA === 'true') {
        ...
        let outputLocal = {
          data: await SensorPouchRepo._select(
            participant_id,
            origin,
            from,
            to,
        ...
        let output = {
          data: await SensorEventRepository._select(
            participant_id,
            origin,
            from,
            to,
