...
export class CredentialService {
  ...
  public static async publicKey(auth: any) {
    // const response: any = await _verify(auth, ["self", "parent"])
...
CredentialService.Router.get("/publicKey", recordStartTime, async (req: Request, res: Response) => {
  const startTime = (req as any).startTime
  res.header(ApiResponseHeaders)
  try {
    const publicKey = await CredentialService.publicKey(req.get("Authorization"))
    const responseTime = Date.now() - startTime
    res.json(publicKey)
    setImmediate(async () => {
      try {
