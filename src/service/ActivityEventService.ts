...
ActivityEventService.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const output = {
        data: await ActivityEventRepository._delete(
          participant_id,
          origin,
          from,
          to,
    ...
    try {
      ...
      // else {
        ...
        let output = {
          data: re2ae(
            req,
            await ActivityEventRepository._select(
              participant_id,
              origin,
              from,
              to,
...
const ae2re = (req: Request, e: any) => {
  if (req.path.endsWith('result_event'))
    ...
    return e.map((x: any) => ({
      ...x,
      temporal_slices: x.temporal_events,
      temporal_events: undefined,
    }));
  return e;
};
const re2ae = (req: Request, e: any) => {
  if (req.path.endsWith('result_event'))
    ...
    return e.map((x: any) => ({
      ...x,
      temporal_events: x.temporal_slices,
      temporal_slices: undefined,
    }));
  return e;
};

export const ResultEventService = Router();
ResultEventService.post(
