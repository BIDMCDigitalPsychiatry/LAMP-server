...
const METHOD_LIST = (auth: any) => ({
  LAMP: {
    ...
    Activity: {
      list: async (study_id: string, ignore_binary = true, sibling = false) => {
        const _start = Date.now()
    ...
    Participant: {
      list: async (study_id: string, sibling = false) => {
        const _start = Date.now()
    ...
    Researcher: {
      ...
      get: async (researcher_id: string, lookup?: boolean, _lookup?: string, study?: string) => {
        const _start = Date.now()
        const x = await ResearcherService.get(auth, researcher_id, lookup, _lookup, study)
    ...
    Study: {
      ...
      get: async (study_id: string, lookup?: boolean, _lookup?: string, mode?: number) => {
        const _start = Date.now()
        const x = await StudyService.get(auth, study_id, lookup, _lookup, mode)
